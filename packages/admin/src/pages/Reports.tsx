...
import { api, fetchText, API_BASE } from '@/lib/api'
import type {
  ReportCategory,
  ReportPeriod,
  ReportResult,
...
const REPORT_TYPES: Record<ReportCategory, Array<{ value: string; label: string }>> = {
  sales: [
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          {meta && (
            <p className="text-sm text-muted-foreground">
              {meta.reportName} · {new Intl.DateTimeFormat('en-US', { dateStyle: 'medium' }).format(new Date(meta.startDate))} – {new Intl.DateTimeFormat('en-US', { dateStyle: 'medium' }).format(new Date(meta.endDate))}
            </p>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" variant="outline" onClick={handleExportCsv} disabled={!table || table.rows.length === 0}>
            <Download />
            Export CSV
          </Button>
        </div>
            ...
            <Field>
              ...
              <Select value={currency} onValueChange={setCurrency}>
                <SelectTrigger id="report-currency" className="w-36">
                  <SelectValue placeholder={meta?.defaultCurrency ?? 'Default'} />
                </SelectTrigger>
                <SelectContent>
        ...
        <>
            ...
            {reportQuery.data.kpis.map((kpi) => (
              <Card key={kpi.key}>
                <CardHeader>
                  ...
                  <CardTitle className="text-2xl font-semibold tabular-nums">{kpi.formattedValue}</CardTitle>
                </CardHeader>
                {kpi.subtext && (
                  <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              {meta?.storeName && <CardDescription>Store: {meta.storeName}</CardDescription>}
            </CardHeader>
            <CardContent>
              {table && table.rows.length > 0 ? (
                ...
                <p className="py-8 text-center text-sm text-muted-foreground">No rows for this report.</p>
              )}
            </CardContent>
          </Card>
        </>
